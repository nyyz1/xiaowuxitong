...
一键发布会完成本机 `db:validate`、`typecheck`、`lint`、`build`，提交并推送 GitHub，触发腾讯云部署，并在结束时校验本地、GitHub `origin/main` 和服务器 commit 一致。
...
8. `npm run db:repair:identity-accounts:dry-run` 预览缺失的身份证号登录账号。
9. `npm run build` 构建生产版本。
10. 重启 `xiaowuxitong.service`。
11. 执行 authenticated page smoke：
...
- 教师和在校学生的身份证号登录账号由导入和页面手工新增流程自动创建；部署时只做 dry-run 预览，不自动补齐历史缺口。
...
如果已有教师或在校学生档案缺失自动登录账号，先确认部署前备份已完成，然后执行：
