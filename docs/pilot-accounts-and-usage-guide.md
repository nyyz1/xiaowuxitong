...
- 新导入或页面手工新增的教师档案会自动创建教师账号，用户名为身份证号，初始密码为身份证号后八位。
- 新导入或页面手工新增的在校学生档案会自动创建学生账号，用户名和初始密码规则同上；往届归档学生不自动创建账号。
...
该入口会把本机验证、GitHub 推送、腾讯云部署和三端 commit 校验串起来。
...
- 首批正式账号创建后，尽快修改临时密码。
- 每次发布优先使用 `publish-and-deploy.cmd`；只部署已推送代码时再使用 `scripts/deploy-tencent-lighthouse.ps1`，避免手工跳过备份或 schema 同步。
- 大批量导入真实数据前先手动备份。
