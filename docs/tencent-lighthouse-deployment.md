...
域名完成备案后，再把域名解析到这台服务器，并把 Nginx 配置从 IP 入口改成域名入口；HTTPS 可以后续通过 Certbot 或学校指定证书方案配置。
...
然后把仓库里的首次部署脚本上传到服务器并执行。脚本会安装 Node.js、PostgreSQL、Nginx，创建应用数据库，克隆 GitHub 仓库，生成 `.env.local`，同步 Prisma schema，初始化基础申请类型和部门岗位，最后启动 systemd 服务。
