...
- `scripts/server/bootstrap-ubuntu.sh` defines the first-time Ubuntu bootstrap path for Tencent Cloud Lighthouse: install Node.js, PostgreSQL, Git, and Caddy; create `/opt/xiaowuxitong`; clone GitHub; generate production `.env.local`; create the local PostgreSQL app database; run Prisma schema sync and baseline seeds; install `xiaowuxitong.service`; and proxy public `:80` to `127.0.0.1:3000`.
